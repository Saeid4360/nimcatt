...
assert.match(homepage, /data-stats-center>آمار/);
assert.match(homepage, /id="statsView"/);
assert.match(homepage, /document\.querySelector\('#statsView > \.stats-hero'\)\?\.remove\(\)/);
...
assert.match(homepage, /id="liveView"/);
assert.match(homepage, /function loadLiveFixtures/);
assert.match(homepage, /function renderLiveSchedule/);
assert.match(homepage, /showView\('live'\)/);
...
const deskPageResponse = await worker.fetch(new Request("https://nimkat.test/editorial-desk.html"), env, ctx);
assert.equal(deskPageResponse.status, 200);
const deskPage = await deskPageResponse.text();
assert.match(deskPage, /اتاق خبر هوشمند نیمکت/);
assert.match(deskPage, /data-view="reader"/);
assert.match(deskPage, /data-view="evaluator"/);
assert.match(deskPage, /data-view="editorial"/);
assert.match(deskPage, /function makeTopics/);
assert.match(deskPage, /function renderEditorial/);
assert.match(deskPage, /\/api\/news-feed/);
...
assert.match(agentPage, /تازه‌سازی خبرها/);
assert.match(agentPage, /limit=\$\{LIVE_LIMIT\}/);
assert.match(agentPage, /id="liveStatus"/);
...
assert.match(workerSource, /function newsFeedClientPayload/);
assert.match(workerSource, /Math\.min\(120/);
assert.match(workerSource, /cacheStatus: "refreshing"/);
